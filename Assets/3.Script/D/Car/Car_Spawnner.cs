...
public class Car_Spawnner : MonoBehaviour {
	[Header("Vehicle 프리팹")]
	...
	private void OnEnable() {
		StartCoroutine(Car_Spawn_co());
	}

	private IEnumerator Car_Spawn_co() {
		while (true) {
			seconds = new WaitForSeconds(Random.Range(min_sec, max_sec));
			Rnd_Set_Pos();
			GameObject car = Instantiate(Vehicle, start_pos);
			car.TryGetComponent(out Car_Controller car_pattern);
			car_pattern.set_pos(start_pos.position, end_pos.position);
			car_pattern.start();

...
public class ThashInfo : MonoBehaviour
{
    ...
    private void ShowInfo(GameObject[] targets)
    {
        ...
        if (infoText != null)
        {
            ...
            // 총 무게
            infoBuilder.AppendLine(isEng ? $"Total: {totalWeight} g" : $"총 무게: {totalWeight} g");

            // 필요 힘 (Max값)
            if (currentStr >= maxReqStr)
            {
                infoBuilder.AppendLine(isEng ? $"Str: {maxReqStr}" : $"필요 힘: {maxReqStr}");
            }
            else
            {
            ...
            if (totalHpCost > 0)
            {
                string costText = isEng ? "CP Loss" : "총 청결도 소모";
